...
impl<F: PrimeField> SparseGrandProductCircuit<F> {
    pub fn construct(leaves: Vec<F>, flags: Vec<bool>) -> Self {
        ...
        for layer in 0..num_layers - 1 {
            let (left, right) = Self::compute_layer(&lefts[layer], &rights[layer], layer_len);
    ...
    fn compute_layer(prior_left: &Vec<SparseEntry<F>>, prior_right: &Vec<SparseEntry<F>>, prior_len: usize) -> (Vec<SparseEntry<F>>, Vec<SparseEntry<F>>) {
